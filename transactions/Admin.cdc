...
transaction(senderAccount: Address, amount: UFix64) {
    ...
    // Define references
    let senderVault: &HotToken.Vault{HotToken.CollectionPublic}
    ...
    let adminResource: &HotToken.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&HotToken.Admin>(from: /storage/AdminStorage)
        ...
        self.senderVault = getAccount(senderAccount)
            ...
            .borrow<&HotToken.Vault{HotToken.CollectionPublic}>()
